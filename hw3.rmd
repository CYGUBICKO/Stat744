...
datatable(working_df, rownames = FALSE)
```
...
temp_df <- (working_df
	...
	%>% filter(grepl("sex", categories, ignore.case = TRUE))
	%>% mutate(cancer_types = reorder(cancer_types, Rate_Ratio, FUN = function(x) -x[1]))
)
ggplot(temp_df, aes(x=years, y=Rate_Ratio, group = 1)) +
    geom_point(alpha=0.4) +
    geom_errorbar(width=.1, aes(ymin=lower, ymax=upper), alpha = 0.4, color = "deepskyblue4") + 
	 facet_wrap(~cancer_types, ncol = 2, scale = "free_y")  




datatable(temp_df, rownames = FALSE)
```
