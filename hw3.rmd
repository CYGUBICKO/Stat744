...
> > - **Mortality rate graphs**: Rates are per 100,000 population and age adjusted to the 2000 US standard population. The first set of Figures show trends in Mortality rates among the poor and the affluent for all the cancer types.
...
plot_poor_affluent_sex <- (ggplot(working_df
	%>% filter(labels %in% c("Affluent", "Poor") & grepl("Sex", grp_cat, ignore.case = TRUE))
		, aes(x=years, y=values, colour = categories))
	+ geom_point(alpha = 0.3)
	+ geom_line(aes(group = categories, fill = categories), alpha = 0.3)
...
plot_poor_affluent_Race <- (ggplot(working_df
...
### Mortality rate plots showing the trends of the estimates for all the cancer types compared across sex and race.
...
### Rate ratio plots showing the ratio estimates together with confidence intervals for all the cancer types compared across sex and race.
